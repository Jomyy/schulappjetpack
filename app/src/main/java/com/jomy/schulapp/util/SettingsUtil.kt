    ...
    fun readSettings(context:Context){
        if(File(context.getExternalFilesDir(null)?.absoluteFile?.path + "/settings.json").exists()){
            try{
                ...
                settings = gson.fromJson(FileReader(context.getExternalFilesDir(null)?.absoluteFile?.path + "/settings.json").readText(),type)
...
fun MutableList<StringSelectSetting>.setByKey(key:String,value:String): MutableList<StringSelectSetting>{
